<%@ tag import="com.jeeplus.common.utils.StringUtils" %>
...
<div class="input-group" style="width:100%">
    ...
    <input type="text" id="${path}" value="${value}"  class="form-control" readonly="readonly"/>

    <span class="input-group-btn">
	       		 <button type="button" id="${path}Button" onclick="window['${path}FileDialogOpen']();" class="btn btn-primary "><i
                         class="fa fa-cloud-upload"></i></button>
       		 </span>
</div>
<script type="text/javascript">
    var v = document.getElementById('${path}').value;
    document.getElementById('${path}').value = jp.getLabelValue(v);

    window['${path}FileDialogOpen'] = function () {
